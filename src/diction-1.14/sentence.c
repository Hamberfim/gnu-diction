  ...
  while ((oc=c)!=EOF)
  {
    ...
    if (length)
    {
      ...
      else
      {
        ...
        else if (length>=4 && !isalpha(sent[length-4]) && !isspace(sent[length-4]) && strncmp(sent+length-3,"...",3)==0 && (c==EOF || isspace(c)))
        {
          /* beginning ellipsis */
          char foo;
          ...
          foo=sent[length-3];
          sent[length-3]='\0';
          if (inSentence) process(sent,length-3,file,beginLine,beginCol,line,col);
          sent[length-3]=foo;
        ...
        else if (length>=4 && strncmp(sent+length-4,"...",3)==0 && (c==EOF || isspace(c)))
        {
          ...
          sent[length]='\0';
          if (inSentence) process(sent,length,file,beginLine,beginCol,line,col);
          length=0;
          inWhiteSpace=0;
          inSentence=0;
        }
    ...
    {
      process("",0,file,beginLine,beginCol,line,col);
      inParagraph=1;
    }
    voc=oc;
  }
  if (!inParagraph) process("",0,file,beginLine,beginCol,line,col);
  regfree(&hashLine);
}
/*}}}*/
